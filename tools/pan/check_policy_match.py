...
def _load_rule_filter(explicit_path: Optional[Path] = None,
                      inline_keywords: Optional[List[str]] = None,
                      disabled: bool = False,
                      ) -> Tuple[List[str], Optional[Path]]:
    """Load the rule-name filter — substrings to exclude from evaluation.

    Resolution order:
      1. If --no-filter was passed → empty filter, no file consulted.
      2. If --rule-filter <path> was passed → read that path (error if missing).
      3. Else look for tools/pan/rule_filter.txt next to this script.
      4. Always combine with any --skip-rule keywords from the CLI.

    Returns (combined_keyword_list, source_file_path_used_or_None).
    Comment lines (#) and blank lines are ignored.
    """
    keywords: List[str] = list(inline_keywords or [])
    ...
    if explicit_path is not None:
        ...
        if not path.exists():
            raise FileNotFoundError(f"--rule-filter file not found: {path}")
        source_path = path
    else:
        default_path = Path(__file__).resolve().parent / "rule_filter.txt"
    ...
    return keywords, source_path


def _rule_matches_filter(rule_name: str, keywords: List[str]) -> Optional[str]:
    """Return the first matching keyword if rule_name contains any of them,
    else None. Substring match, CASE-INSENSITIVE — 'icmp' matches both
    'ICMP-allow' and 'icmp_ping_out'."""
    rule_lower = rule_name.lower()
    for kw in keywords:
        if kw and kw.lower() in rule_lower:
            return kw
    return None
...
def evaluate(config: PanoramaConfig, query: Query,
             rule_filter: Optional[List[str]] = None) -> Verdict:
    """Walk the evaluation chain and return a verdict.

    rule_filter: optional list of substrings. Any rule whose name contains
    any of these is REMOVED FROM THE CHAIN before evaluation — it does not
    appear in the trace, does not contribute to rules_evaluated, and does
    not influence the verdict. The count of filtered rules is recorded
    separately as a caveat so consumers know it happened.
    """
    chain = config.evaluation_chain(query.device_group)
    rule_filter = rule_filter or []
    if rule_filter:
        filtered_count = sum(
            1 for r in chain if _rule_matches_filter(r.name, rule_filter)
        )
        chain = [r for r in chain
                 if not _rule_matches_filter(r.name, rule_filter)]
    else:
        filtered_count = 0
    ...
    if filtered_count > 0:
        cumulative_caveats.append(
            f"{filtered_count} rule(s) removed from evaluation by rule_filter; "
            f"the matcher behaved as if those rules did not exist."
        )
...
def main() -> int:
    ...
    dg_grp.add_argument("--all-device-groups", action="store_true",
                        help="Evaluate the query against EVERY device-group in the "
                             "config and report a per-DG verdict. This is the DEFAULT "
                             "when neither --device-group nor --all-device-groups is "
                             "given.")
    ...
    p.add_argument("--rule-filter", default=None,
                   help="Path to a rule-filter file. Each non-comment line is "
                        "a substring; any rule whose name contains a substring "
                        "is removed from evaluation. Default: tools/pan/rule_filter.txt "
                        "if present.")
    p.add_argument("--skip-rule", action="append", default=[],
                   metavar="KEYWORD",
                   help="Additional inline filter keyword. Repeatable. Combined "
                        "with file-based filter. Substring match on rule name.")
    p.add_argument("--no-filter", action="store_true",
                   help="Disable rule filtering for this run even if "
    ...
    # ----- Load rule filter (file + inline keywords) -----
    try:
        rule_filter, filter_source = _load_rule_filter(
    ...
    # Default to --all-device-groups when neither flag is given. Keeps CAB
    # workflows simple: "is this flow allowed anywhere?" works without
    # operator guessing a DG name.
    if not args.device_group and not args.all_device_groups:
        log.info("No --device-group or --all-device-groups specified; "
                 "defaulting to --all-device-groups.")
    ...
    for dg in target_dgs:
        ...
        verdicts.append((dg, evaluate(config, q, rule_filter=rule_filter)))
    ...
    if not args.no_disk:
        ...
        if args.all_device_groups:
            # In all-DG mode the trace would be N × the size of single-DG.
            # Without -v, strip the per-DG trace to keep the file readable.
            def _per_dg_doc(dg: str, v: Verdict) -> Dict[str, Any]:
                doc = {"device_group": dg, **verdict_to_json(v)}
                if not args.verbose:
                    doc.pop("trace", None)
                return doc
            ...
            doc = {
                ...
                "trace_included": bool(args.verbose),
                "src_ip": args.src_ip, "dst_ip": args.dst_ip,
                "src_zone": args.src_zone, "dst_zone": args.dst_zone,
                "protocol": args.protocol, "dst_port": args.dst_port,
                "device_groups": [_per_dg_doc(dg, v) for dg, v in verdicts],
            }
        else:
            # Single-DG mode keeps full trace in the file by default — that's
            # one rulebase per query, and the trace is the audit record for the
            # "why did this match" CAB question.
    ...
    if args.json:
        if args.all_device_groups:
            def _per_dg_doc(dg: str, v: Verdict) -> Dict[str, Any]:
                d = {"device_group": dg, **verdict_to_json(v)}
                if not args.verbose:
                    d.pop("trace", None)
                return d
            print(json.dumps({
