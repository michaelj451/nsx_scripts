...
| `/remap-pivot` | Same CSV remap dry-run data as `/group-remap`, pivoted: one row per group/rule with Source (adds) and Destination (adds) as parallel columns. Shares run history with `/group-remap` |
