# Runbook — Rules Usage Report (macOS / Linux / bash)
...
PowerShell variant: [RUNBOOK_RULES_USAGE_PS.md](RUNBOOK_RULES_USAGE_PS.md).
...
| Convention | Value |
|---|---|
| Target selection | `--target nsx-lm1` (aliases from `app/nsx/nsx_constants.py`) |
...
| Log directory | `$NSX_LOG_DIR` from `.env` (resolves to `nsx_logs/` by default) |
| Report location | `$NSX_LOG_DIR/rules_usage_report/<host>/<UTC_TS>/` |
...
```
$NSX_LOG_DIR/rules_usage_report/nsx-lm1.lab.local/<UTC_TS>/
├── summary.json
├── rules_usage.json
├── rules_usage.jsonl
├── hot_rules.json
...
```bash
cat $NSX_LOG_DIR/rules_usage_report/nsx-lm1.lab.local/<UTC_TS>/summary.json
```
...
**Recommendation: weekly cron.**

```cron
0 2 * * 0 cd /Users/mxferguson/dev/nsx_scripts && \
  /Users/mxferguson/dev/nsx_scripts/.venv/bin/python tools/reports/report_rules_usage.py \
  --target nsx-lm1 >> $HOME/cron-rules-usage.log 2>&1
```

History grows in place — each weekly run lands a new timestamped subdirectory
under `$NSX_LOG_DIR/rules_usage_report/nsx-lm1.lab.local/`. The tool
...
```bash
python tools/reports/report_rules_usage.py --target nsx-lm1 \
  --compare-to nsx_logs/rules_usage_report/nsx-lm1.lab.local/<prior-UTC_TS>/
```
...
jq 'select(.classification == "STALE")' \
  $NSX_LOG_DIR/rules_usage_report/nsx-gm1.lab.local/<UTC_TS>/rules_usage.jsonl

# Per-domain HOT counts (from the federation walk)
jq '.per_domain | to_entries | map({domain: .key, HOT: .value.HOT})' \
  $NSX_LOG_DIR/rules_usage_report/nsx-gm1.lab.local/<UTC_TS>/summary.json
```
...
| Local filesystem | Writes only under `$NSX_LOG_DIR/rules_usage_report/<host>/<UTC_TS>/`; never modifies capture bundles or any other tool's output |
...
cat $NSX_LOG_DIR/rules_usage_report/nsx-lm1.lab.local/<UTC_TS>/no_hits_in_n_days.json
```
...
```bash
jq -s 'sort_by(.classification, -.hit_count) | .[] |
  select(.classification == "HOT" or .classification == "STALE") |
  {classification, domain_id, rule_id, hit_count, days_since_hit_changed}' \
  $NSX_LOG_DIR/rules_usage_report/nsx-lm1.lab.local/<UTC_TS>/rules_usage.jsonl
```
...
# After
python tools/reports/report_rules_usage.py --target nsx-lm1 --compare-to "$SNAP_BEFORE"
SNAP_AFTER=$(ls -1dt $NSX_LOG_DIR/rules_usage_report/nsx-lm1.lab.local/* | head -1)
jq '.transitions[] | select(.transition == "lit_up")' "$SNAP_AFTER/diff.json"
```
...
diff <(jq -S . $NSX_LOG_DIR/rules_usage_report/nsx-lm1.lab.local/<ts>/summary.json) \
     <(jq -S . $NSX_LOG_DIR/rules_usage_report/nsx-lm2.lab.local/<ts>/summary.json)
```
...
| `--history-dir <path>` | `$NSX_LOG_DIR/rules_usage_report/<host>/` | Override history scan root |
| `--min-days-since-hit <int>` | (off) | Emit `no_hits_in_n_days.json` filter |
| `--compare-to <path>` | (off) | Diff against a prior snapshot directory |
| `--output-base <path>` | `$NSX_LOG_DIR` | Override report root (testing) |
